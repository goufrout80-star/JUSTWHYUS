...
const platformOpts = ['YouTube', 'TikTok', 'Instagram', 'Twitter / X', 'LinkedIn', 'Other']
const nicheOpts = ['AI & Tech', 'Video Editing', 'Filmmaking', 'Productivity', 'Creative Software', 'SaaS & Tools']
const audienceOpts = ['Under 5,000', '5K \u2013 25K', '25K \u2013 100K', '100K \u2013 500K', '500K+']

export default function Creators() {
  useDocumentHead({
    title: 'For Creators — Apply to Our Network | JUST WHY US',
    description: 'Join a vetted creator network and get matched to real brand campaigns. No cold pitching. Paid on time, every time.',
    canonical: 'https://justwhyus.com/creators',
  })
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const { error } = await supabase.from('creator_applications').insert({
      name: f.answers.name?.trim() || '',
      handle: f.answers.handle?.trim() || null,
      platform: f.answers.platform || null,
      niche: f.answers.niche || null,
      audience_size: f.answers.audience || null,
      email: f.answers.email?.trim() || '',
      best_content_link: f.answers.content_link?.trim() || null,
      status: 'pending',
  ...
  return (
    <div style={{ height: '100dvh', backgroundColor: VOID, overflow: 'hidden', position: 'relative' }}>
      <DotGrid />
      {f.currentStep > 0 && <TopBar step={f.currentStep} total={TOTAL} onBack={f.prevStep} accent={CORAL} />}
      ...
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 24px' }}>
        <AnimatePresence mode="wait">
          {f.currentStep === 0 && (
            <Step key="intro">
              ...
              <p className="font-italic" style={{ fontSize: 16, color: 'rgba(240,235,216,0.45)', marginTop: 12, lineHeight: 1.7 }}>
                We bring the work to you.<br />Answer 5 quick questions.
              </p>
              <Btn accent={CORAL} onClick={f.nextStep} label="APPLY NOW" />
            </Step>
          )}

          {f.currentStep === 1 && (
            <Step key="s1">
              <Label text="Who are you" accent={CORAL} />
              <Question text="What's your name and handle?" />
              <TextInput
                value={f.answers.name || ''}
                onChange={(v) => f.setAnswer('name', v)}
                placeholder="Your name"
                accent={CORAL}
                autoFocus
              />
              <TextInput
                value={f.answers.handle || ''}
                onChange={(v) => f.setAnswer('handle', v)}
                placeholder="@yourhandle"
                accent={CORAL}
                onEnter={() => f.answers.name?.trim() && f.nextStep()}
                style={{ marginTop: 16 }}
              />
              <Hint />
            </Step>
          ...
          {f.currentStep === 2 && (
            <Step key="s2">
              <Label text="Your platform" accent={CORAL} />
              <Question text="Where do you create?" />
              <OptionGrid
                options={platformOpts}
                selected={f.answers.platform || ''}
                accent={CORAL}
                onSelect={(v) => { f.setAnswer('platform', v); setTimeout(f.nextStep, 200) }}
              />
            </Step>
          )}
          ...
          {f.currentStep === 3 && (
            <Step key="s3">
              <Label text="Your niche" accent={CORAL} />
              <Question text="What's your content about?" />
              <OptionGrid
                options={nicheOpts}
          ...
          {f.currentStep === 4 && (
            <Step key="s4">
              <Label text="Your audience" accent={CORAL} />
              <Question text="How big is your audience?" />
              <OptionGrid
                options={audienceOpts}
                selected={f.answers.audience || ''}
                accent={CORAL}
                cols={3}
                onSelect={(v) => { f.setAnswer('audience', v); setTimeout(f.nextStep, 200) }}
              />
            </Step>
          )}
          ...
          {f.currentStep === 5 && (
            <Step key="s5">
              <Label text="Last thing" accent={CORAL} />
              <Question text="How do we reach you?" />
              <TextInput
              ...
              />
              <TextInput
                value={f.answers.content_link || ''}
                onChange={(v) => f.setAnswer('content_link', v)}
                placeholder="https://..."
                accent={CORAL}
              ...
              <Btn accent={CORAL} onClick={handleSubmit} label={f.isSubmitting ? 'SENDING...' : 'SEND APPLICATION'} disabled={f.isSubmitting || !f.answers.email?.trim()} />
            </Step>
          )}
        </AnimatePresence>
      </div>
  ...
  return (
    <div style={{ height: '100dvh', backgroundColor: VOID, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', textAlign: 'center', padding: '0 24px' }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} style={{ position: 'relative' }}>
        <motion.svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke={CORAL} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ margin: '0 auto 24px' }}>
        ...
        <h1 className="font-brand" style={{ fontSize: 'clamp(36px,5vw,56px)', color: CREAM }}>APPLICATION SENT.</h1>
        <p className="font-italic" style={{ fontSize: 15, color: 'rgba(240,235,216,0.4)', marginTop: 12, lineHeight: 1.7 }}>
          We&rsquo;ll reach out if your profile<br />matches an active campaign.
        </p>
        <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 400, fontSize: 10, letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(240,235,216,0.18)', marginTop: 24 }}>
          NO MASS EMAILS.<br />DIRECT CONTACT ONLY.
        </p>
      </motion.div>
    </div>
  )
...
function ErrMsg({ msg }: { msg: string }) {
  return (
    <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 400, fontSize: 12, color: CORAL, marginTop: 12 }}>
      {msg}
    </p>
  )
}
